...
(defn init-player! [dom paused playback on-progress config]
...
(defn player [{:keys [paused playback on-progress config]}]
  (let [pl (atom nil)]
    (as-component {
                    :on-mount (fn [this] (reset! pl (init-player! (r/dom-node this) paused playback on-progress config)))
